...
# PyInstaller 환경/개발 환경 모두에서 BASE_DIR 자동 설정
...
# --- 기본 경로 설정 ---
# main.py가 ericapython 루트에 위치할 때를 기준으로 BASE_DIR 설정
BASE_DIR = Path(__file__).resolve().parent

# assets, data 등은 BASE_DIR / 'assets' / ... 형태로 접근

# --- 엔딩 달성 업적 저장/로드 ---
ENDING_ACHIEVE_FILE = BASE_DIR / "data" / "ending_achieve.json"
def load_ending_achieved():
    try:
        with open(ENDING_ACHIEVE_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
...
def save_ending_achieved(ending_achieved):
    try:
        with open(ENDING_ACHIEVE_FILE, "w", encoding="utf-8") as f:
            json.dump(ending_achieved, f)
    except Exception as e:
        print(f"엔딩 업적 저장 오류: {e}")

# 전역 엔딩 업적 상태
ending_achieved = load_ending_achieved()
def draw_achievement_screen():
    global ending_achieved
    # 초기화 버튼 위치 및 크기
    reset_btn_width, reset_btn_height = 120, 48
    reset_btn_rect = pygame.Rect(WIDTH - reset_btn_width - 40, HEIGHT - reset_btn_height - 32, reset_btn_width, reset_btn_height)
    pygame.draw.rect(screen, (220, 60, 60), reset_btn_rect, border_radius=12)
    reset_font = pygame.font.SysFont('malgungothic', 28, bold=True)
    reset_text = reset_font.render("초기화", True, (255,255,255))
    reset_text_rect = reset_text.get_rect(center=reset_btn_rect.center)
    screen.blit(reset_text, reset_text_rect)
    global achievement_reset_btn_rect
    achievement_reset_btn_rect = reset_btn_rect

    # 클릭된 엔딩 인덱스 저장용(전역)
    global achievement_selected_idx
    if 'achievement_selected_idx' not in globals():
        achievement_selected_idx = None
    # 배경에 게임 화면을 먼저 그림 (상점/설정과 동일)
    draw_game_screen()
    # 설정/상점과 동일하게 전체 화면에 반투명 오버레이를 먼저 그림
    overlay = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
    overlay.fill((0,0,0,180))
    screen.blit(overlay, (0,0))
    # 중앙에 흰색 박스(설정/상점 스타일) - 패널 크기 먼저 계산
    panel_width = 900
    panel_height = 400
    panel_rect = pygame.Rect((WIDTH - panel_width)//2, (HEIGHT - panel_height)//2, panel_width, panel_height)
    # 패널은 불투명 흰색 Surface로 만듦
    panel_surface = pygame.Surface((panel_width, panel_height), pygame.SRCALPHA)
    panel_surface.fill((255,255,255,255))
    # 닫기 버튼 (오른쪽 상단)
    close_btn_size = 40
    close_btn_rect = pygame.Rect(panel_rect.right - close_btn_size - 16, panel_rect.top + 16, close_btn_size, close_btn_size)
    pygame.draw.rect(panel_surface, (220,60,60), pygame.Rect(panel_width - close_btn_size - 16, 16, close_btn_size, close_btn_size), border_radius=8)
    x_font = pygame.font.SysFont('malgungothic', 32, bold=True)
    x_surf = x_font.render('X', True, (255,255,255))
    x_rect = x_surf.get_rect(center=(panel_width - close_btn_size//2 - 16, 16 + close_btn_size//2))
    panel_surface.blit(x_surf, x_rect)
    # 업적(엔딩) 패널 상단 중앙에 '엔딩 업적' 문구 추가
    title_font = pygame.font.SysFont('malgungothic', 40, bold=True)
    title_text = "엔딩 업적"
    title_surf = title_font.render(title_text, True, (60, 60, 60))
    title_rect = title_surf.get_rect(center=(panel_width//2, 36))
    panel_surface.blit(title_surf, title_rect)

    margin = 30
    # panel_rect 안에 5개 이미지가 margin 포함 모두 들어가도록 크기 자동 조정
    available_width = panel_width - margin * 4
    img_w = available_width // 5
    img_h = int(img_w * ENDING_IMAGE_SIZE[1] / ENDING_IMAGE_SIZE[0])
    # 높이가 panel_rect를 넘으면 높이에 맞춰 다시 조정
    max_img_h = panel_height - 100
    if img_h > max_img_h:
        img_h = max_img_h
        img_w = int(img_h * ENDING_IMAGE_SIZE[0] / ENDING_IMAGE_SIZE[1])
    total_width = img_w * 5 + margin * 4
    start_x = panel_rect.left + (panel_width - total_width)//2
    y = panel_rect.top + 60
    # 엔딩 이미지는 panel_rect 내부에만 그리도록 클리핑
    # 엔딩 달성 여부 리스트(예시: 실제로는 세이브 데이터 등에서 불러와야 함)
    # 예시: [True, False, False, True, False] (엔딩1,4만 달성)
    global ending_achieved
    # 엔딩 이미지 rect 리스트 저장(클릭 판정용)
    global achievement_img_rects
    achievement_img_rects = []
    for i, img in enumerate(ending_images_loaded):
        scaled_img = pygame.transform.smoothscale(img, (img_w, img_h))
        rect = scaled_img.get_rect()
        rect.topleft = ( (panel_width - total_width)//2 + i*(img_w+margin), 60 )
        achievement_img_rects.append(rect.move(panel_rect.left, panel_rect.top))
        if ending_achieved[i]:
            panel_surface.blit(scaled_img, rect)
        else:
            dark_img = scaled_img.copy()
            dark_overlay = pygame.Surface((img_w, img_h), pygame.SRCALPHA)
            dark_overlay.fill((30,30,30,235))  # 더 어둡고 불투명하게
            dark_img.blit(dark_overlay, (0,0), special_flags=pygame.BLEND_RGBA_MULT)
            panel_surface.blit(dark_img, rect)
        pygame.draw.rect(panel_surface, (0,0,0), rect, 3)
    screen.blit(panel_surface, panel_rect.topleft)

    # 엔딩 해금 조건 문구 표시
    unlock_conditions = [
        "해금 조건 : 520일차 이하 10억달성",
        "해금 조건 : 521~550일차안에 10억달성",
        "해금 조건 : 551~590일차안에 10억달성",
        "해금 조건 : 591~630일차안에 10억달성",
        "해금 조건 : 631일차 이상 10억달성"
    ]
    ending_titles = [
        "전설의 농부",
        "자연과의 조회",
    ...
    ending_colors = [
        (70, 170, 255),   # 첫 수확의 기쁨 - 파랑
        (60, 180, 80),    # 농장의 성장 - 초록
        (255, 200, 40),   # 황금빛 수확 - 노랑
        (180, 120, 255),  # 자연과의 조회 - 보라
        (255, 100, 60)    # 전설의 농부 - 주황
    ]
    if achievement_selected_idx is not None:
        cond_font = pygame.font.SysFont('malgungothic', 32, bold=True)
        if ending_achieved[achievement_selected_idx]:
            # 엔딩 달성 시 문구와 색상 표시
            title_text = ending_titles[achievement_selected_idx]
            title_color = ending_colors[achievement_selected_idx]
            title_surf = cond_font.render(title_text, True, title_color)
            title_rect = title_surf.get_rect(center=(panel_rect.centerx, panel_rect.bottom - 40))
            screen.blit(title_surf, title_rect)
        else:
            cond_text = unlock_conditions[achievement_selected_idx]
            cond_surf = cond_font.render(cond_text, True, (60,60,60))
            cond_rect = cond_surf.get_rect(center=(panel_rect.centerx, panel_rect.bottom - 40))
            screen.blit(cond_surf, cond_rect)
    # 닫기 버튼 클릭 영역을 전역 변수로 저장
    global achievement_close_btn_rect
    achievement_close_btn_rect = close_btn_rect
DAY_CHANGE_INTERVAL = 2
original_day_change_interval = DAY_CHANGE_INTERVAL  # 날짜 변경 기본값 저장
fast_day_mode = False  # F2로 토글되는 빠른 날짜 모드
original_day_change_interval = DAY_CHANGE_INTERVAL  # 날짜 변경 기본값 저장
fast_day_mode = False  # F2로 토글되는 빠른 날짜 모드
game_start_time = None  # 게임 시작 시간(초)
...
def draw_skip_circle(screen, progress):
    ...
    try:
        ...
        font = pygame.font.SysFont('Galmuri14', 20)
    # 문구 렌더링
    text = font.render('E를 꾹 눌러 스킵', True, (255, 255, 180))
        ...
        try:
            with open(SETTINGS_FILE, "r") as f:
                ...
                for k, v in DEFAULT_SETTINGS.items():
                    if k not in loaded:
                        loaded[k] = v
                if "master_volume" not in loaded:
                    loaded["master_volume"] = 1
                if "music_volume" not in loaded:
                    loaded["music_volume"] = 1
                if "sfx_volume" not in loaded:
                    loaded["sfx_volume"] = 1
                return loaded
        except Exception as e:
...
achievement_icon_path = BASE_DIR / "assets" / "images" / "achievement_icon.png"
ACHIEVEMENT_ICON_SIZE = 50
try:
    achievement_icon = pygame.image.load(achievement_icon_path)
    achievement_icon = pygame.transform.scale(achievement_icon, (ACHIEVEMENT_ICON_SIZE, ACHIEVEMENT_ICON_SIZE))
except pygame.error as e:
    print(f"업적 아이콘 로드 오류 {achievement_icon_path}: {e}")
    print("업적 아이콘 파일이 'assets/images' 폴더에 있는지 확인하세요. (예: achievement_icon.png)")

# 업적 UI 상태 변수
show_achievement_ui = False
ACHIEVEMENT_STATE = 'ACHIEVEMENT'

# 엔딩 이미지 및 설명 (예시)
ENDING_IMAGE_SIZE = (180, 140)
ENDING_IMAGE_PATHS = [
    BASE_DIR / "assets" / "images" / f"ending{i}.png" for i in range(1, 6)
]
ENDING_DESCRIPTIONS = [
    "엔딩1: 520일차안에 소지금 10억 이상 달성!",
    "엔딩2: 550일차안에 소지금 10억 이상 달성!",
    "엔딩3: 30분 이하에 소지금 10억 이상 달성!",
    "엔딩4: 35분 이하에 소지금 10억 이상 달성!",
    "엔딩5: 40분 이하에 소지금 10억 이상 달성!",
]
ending_images_loaded = []
for path in ENDING_IMAGE_PATHS:
    try:
        img = pygame.image.load(path).convert_alpha()
        img = pygame.transform.smoothscale(img, ENDING_IMAGE_SIZE)
        ending_images_loaded.append(img)
    except Exception:
        # 이미지가 없으면 빈 Surface
        surf = pygame.Surface(ENDING_IMAGE_SIZE)
        surf.fill((220,220,220))
        ending_images_loaded.append(surf)
...
# 식물 성장 프레임
plant_sunflower_growth_frames = 15
plant_lettuce_growth_frames = 25
plant_potato_growth_frames = 30
plant_pea_growth_frames = 50
...
try:
    ...
    font = pygame.font.SysFont('Galmuri14', 30)
    price_text_font = pygame.font.SysFont('Galmuri14', 25) # 가격 텍스트 폰트
    multiplier_text_font = pygame.font.SysFont('Galmuri14', 20) # 배율 텍스트 폰트
    title_font = pygame.font.SysFont('Galmuri14', 70)
    button_font = pygame.font.SysFont('Galmuri14', 40)
    settings_title_font = pygame.font.SysFont('Galmuri14', 50)
    volume_label_font = pygame.font.SysFont('Galmuri14', 30)
    shop_title_font = pygame.font.SysFont('Galmuri14', 50)
    seed_name_font = pygame.font.SysFont('Galmuri14', 20)
    cooldown_font = pygame.font.SysFont('Galmuri14', 40)
    tool_shop_title_font = pygame.font.SysFont('Galmuri14', 50) # 대체 폰트도 함께 설정
    tool_shop_pirce_font = pygame.font.SysFont('Galmuri14', 20) # 도구 상점 가격 폰트
    description_font = pygame.font.SysFont('Galmuri14', 18) # 대체 폰트도 함께 설정
except Exception as e:
...
class EndingManager:
    ...
    def __init__(self):
        self.endings = [
            {"condition": lambda gd: gd.get("money", 0) >= 1_000_000_000 and gd.get("days", 0) <= 520, "message": "엔딩1: 520일차안에 소지금 10억 이상 달성!", "text": [
                "플레이어의 농장은 거대한 농업 제국이 되어,\n 마을을 넘어 세계로 영향력을 뻗친다.",
                "용과는 희귀한 고급 과일로 세계 각지에 수출되고,\n 해바라기는 에너지 자원으로 쓰이며",
                "완두콩과 감자는 인류를 먹여 살리는 식량이 된다.",
                "후손들은 당신을 전설의 농부로 기억한다.",
                "엔딩1: 씨앗에서 시작된 이야기는 인류의 미래가 되었다."
            ]},
            {"condition": lambda gd: gd.get("money", 0) >= 1_000_000_000 and 520 < gd.get("days", 0) <= 550, "message": "엔딩2: 550일차안에 소지금 10억 이상 달성!", "text": [
                "플레이어는 농사를 통해 자연과  소통하는 법을 깨닫는다.",
                "용과 나무와 해바라기 밭, 완두콩 덩굴, 감자밭이 \n아름다운 풍경을 만들어 마치 작은 낙원이 된다.",
                "농장은 이제 단순한 돈벌이가\n아니라 자연과 인간이 공존하는 상징이 된다.",
                "엔딩2 : 땅을 가꾼 자,세상을 가꾼다."
            ]},
            {"condition": lambda gd: gd.get("money", 0) >= 1_000_000_000 and 550 < gd.get("days", 0) <= 590, "message": "엔딩3: 590일차안에 소지금 10억 이상 달성!", "text": [
                "해바라기 밭이 끝없이 펼쳐지고,\n수확한 씨앗이 황금빛처럼 빛난다.",
                "농장은 단순한 생계 수단이\n아니라 부와 명예의 상징이 된다.",
                "당신은 '황금농부'라는 별명을 얻는다.",
                "엔딩3 : 노력의 결실은 황금보다 빛난다."
            ]},
            {"condition": lambda gd: gd.get("money", 0) >= 1_000_000_000 and 590 < gd.get("days", 0) <= 630, "message": "엔딩4: 630일차안에 소지금 10억 이상 달성!", "text": [
                "밭이 확장되고 작물의 종류가 다양해진다.",
                "완두콩과 감자를 대량으로\n수확해 마을 시장에 공급한다.",
                "마을 경제가 활기를 띄며\n농부의 이름이 널리 알려진다.",
                "엔딩4 : 너의 땀방울은 모두의 풍요가 되었다."
            ]},
            {"condition": lambda gd: gd.get("money", 0) >= 1_000_000_000 and gd.get("days", 0) > 630, "message": "엔딩5: 631일차 이후 소지금 10억 이상 달성!", "text": [
                "당신은 작은 밭에서 첫 작물들을 수확한다.",
                "해바라기의 씨앗을 팔아 초기 자금을 마련하고,\n용과 한송이를 처음으로 재배하는 데 성공.",
                "마을 사람들이 찾아와\n\"이제 진짜 농부가 되었구나!\"라며 축하한다.",
                "엔딩5 : 작은 시작이 큰 농부의 길이 된다."
            ]},
        ]

    def check_endings(self, game_data):
        for idx, ending in enumerate(self.endings):
            if ending["condition"](game_data):
                # 엔딩 달성 시 업적 저장
                global ending_achieved
                if not ending_achieved[idx]:
                    ending_achieved[idx] = True
                    save_ending_achieved(ending_achieved)
                self.show_ending(ending["message"], ending["text"])
                return True
        return False

    def time_money_condition(self, game_data, time_limit):
        # time_limit(초) 내 10억 달성
        return game_data.get("money", 0) >= 1_000_000_000 and game_data.get("play_time", 999999) <= time_limit

    def show_ending(self, message, text):
        # 디버깅용: 어떤 엔딩이 재생되는지 출력
        debug_msg = f"[DEBUG] {message.split(':')[0]}이 재생됩니다"
        print(debug_msg)
        import pygame
        import os
        import re
        ...
        for alpha in range(0, 256, 8):  # 더 빠른 페이드(알파 간격 증가)
        ...
        # 엔딩 이미지 페이드 인 효과 (엔딩 번호별 이미지)
        try:
            match = re.match(r"엔딩(\d+)", message)
            ending_num = match.group(1) if match else "1"
            ending_img_path = os.path.join("assets", "images", f"ending{ending_num}.png")
            ending_img = pygame.image.load(ending_img_path).convert_alpha()
            # 엔딩1만 이미지 크기 축소(80%) 및 중앙 배치
            if ending_num == "1":
                scale = 0.8
                img_w, img_h = ending_img.get_size()
                new_w, new_h = int(img_w * scale), int(img_h * scale)
                ending_img = pygame.transform.smoothscale(ending_img, (new_w, new_h))
                img_pos = ((screen.get_width() - new_w) // 2, (screen.get_height() - new_h) // 2)
            else:
                ending_img = pygame.transform.scale(ending_img, screen.get_size())
                img_pos = (0, 0)
            for alpha in range(0, 256, 8):
                fade_surface.set_alpha(255)
                screen.blit(fade_surface, (0, 0))
                img_surface = ending_img.copy()
                img_surface.set_alpha(alpha)
                screen.blit(img_surface, img_pos)
                pygame.display.update()
                pygame.event.pump()
                pygame.time.delay(16)
        except Exception as e:
            print(f"엔딩 이미지 로드 오류: {e}")

        # 타이핑 효과로 엔딩 텍스트 출력 (여러 줄 지원)
        font = pygame.font.Font(os.path.join("assets", "fonts", "Galmuri14.ttf"), 36)
        text_color = (255, 255, 255)
        x, y = 60, screen.get_height() - 260
        line_spacing = 48
        if isinstance(text, str):
            text = [text]
        for line in text:
            sublines = line.split("\n")
            # 줄바꿈 문자로 정확히 분리 (윈도우/유니코드 호환)
            if '\n' in line:
                sublines = line.split('\n')
            else:
                sublines = [line]
            if len(sublines) == 2:
                ...
                while typing0:
                    for event in pygame.event.get():
                        ...
                        if event.type == pygame.KEYDOWN or event.type == pygame.MOUSEBUTTONDOWN:
                            # 입력 시 즉시 전체 문장 출력
                            displayed_lines[0] = sublines[0]
                            char_idx0 = len(sublines[0])
                            typing0 = False
                    if char_idx0 < len(sublines[0]):
                        displayed_lines[0] += sublines[0][char_idx0]
                        char_idx0 += 1
                    screen.blit(fade_surface, (0, 0))
                    try:
                        screen.blit(ending_img, img_pos)
                    except:
                        pass
                    txt_surf0 = font.render(displayed_lines[0], True, text_color)
                    txt_rect0 = txt_surf0.get_rect(center=(screen.get_width() // 2, y))
                    screen.blit(txt_surf0, txt_rect0)
                    pygame.display.update()
                    pygame.event.pump()
                    pygame.time.delay(16)
                    if char_idx0 >= len(sublines[0]):
                        typing0 = False
                # 첫 줄이 모두 나온 뒤 바로 두 번째 줄 타이핑 효과로 출력
                typing1 = True
                char_idx1 = 0
                while typing1:
                    skip = False
                    for event in pygame.event.get():
                        if event.type == pygame.QUIT:
                            ...
                            exit()
                        if event.type == pygame.KEYDOWN or event.type == pygame.MOUSEBUTTONDOWN:
                            displayed_lines[1] = sublines[1]
                            char_idx1 = len(sublines[1])
                            typing1 = False
                    if char_idx1 < len(sublines[1]):
                        displayed_lines[1] += sublines[1][char_idx1]
                        char_idx1 += 1
                    else:
                        typing1 = False
                    screen.blit(fade_surface, (0, 0))
                    ...
                    txt_surf0 = font.render(displayed_lines[0], True, text_color)
                    txt_rect0 = txt_surf0.get_rect(center=(screen.get_width() // 2, y))
                    txt_surf1 = font.render(displayed_lines[1], True, text_color)
                    txt_rect1 = txt_surf1.get_rect(center=(screen.get_width() // 2, y + line_spacing))
                    screen.blit(txt_surf0, txt_rect0)
                    screen.blit(txt_surf1, txt_rect1)
                    pygame.display.update()
                    pygame.event.pump()
                    pygame.time.delay(16)
                # 두 줄 모두 다 나온 뒤에 입력을 기다림
                pygame.event.clear()
                waiting = True
                while waiting:
                    for event in pygame.event.get():
                ...
                for subline in sublines:
                    is_last_ending1 = (subline.startswith("엔딩1:"))
                    # 엔딩3의 '황금농부'만 노란색으로 출력
                    if "'황금농부'" in subline:
                        before, keyword, after = subline.partition("'황금농부'")
                        displayed = ""
                        typing = True
                        char_idx = 0
                        while True:
                            ...
                            if typing:
                                if skip:
                                    displayed = before + keyword + after
                                    char_idx = len(before + keyword + after)
                                else:
                                    if char_idx < len(before):
                                        displayed += before[char_idx]
                                        char_idx += 1
                                    elif char_idx < len(before) + len(keyword):
                                        # '황금농부' 타이핑
                                        displayed += keyword[char_idx - len(before)]
                                        char_idx += 1
                                    elif char_idx < len(before) + len(keyword) + len(after):
                                        displayed += after[char_idx - len(before) - len(keyword)]
                                        char_idx += 1
                                    else:
                                        typing = False
                            screen.blit(fade_surface, (0, 0))
                            try:
                                ...
                                pass
                            # 앞부분
                            txt_surf1 = font.render(before, True, text_color)
                            txt_rect1 = txt_surf1.get_rect()
                            # '황금농부'
                            txt_surf2 = font.render(keyword, True, (255, 255, 0))
                            txt_rect2 = txt_surf2.get_rect()
                            # 뒷부분
                            txt_surf3 = font.render(after, True, text_color)
                            txt_rect3 = txt_surf3.get_rect()
                            # 전체 중앙 정렬
                            total_width = txt_rect1.width + txt_rect2.width + txt_rect3.width
                            start_x = (screen.get_width() - total_width) // 2
                            y_pos = y + font.get_height() // 2
                            screen.blit(txt_surf1, (start_x, y_pos))
                            screen.blit(txt_surf2, (start_x + txt_rect1.width, y_pos))
                            screen.blit(txt_surf3, (start_x + txt_rect1.width + txt_rect2.width, y_pos))
                            pygame.display.update()
                            pygame.event.pump()
                            if not typing:
                                break
                            pygame.time.delay(16)
                        if not is_last_ending1:
                            pygame.event.clear()
                    else:
                        displayed = ""
                        typing = True
                        ...
                        while True:
                            ...
                            if typing:
                                if skip:
                                    displayed = subline
                                    char_idx = len(subline)
                                else:
                                    if char_idx < len(subline):
                                        displayed += subline[char_idx]
                                        char_idx += 1
                                    else:
                                        typing = False
                            screen.blit(fade_surface, (0, 0))
                            try:
                                ...
                                pass
                            txt_surf = font.render(displayed, True, text_color)
                            txt_rect = txt_surf.get_rect(center=(screen.get_width() // 2, y + font.get_height() // 2))
                        ...
                        if not is_last_ending1:
                            ...
                            pygame.quit()
                            sys.exit()
                # 타이핑이 끝난 후 엔딩1 마지막 문구일 때 조건 문구 없이 화면 유지
                if is_last_ending1:
                    keep_displaying = True
                    while keep_displaying:
                        screen.blit(fade_surface, (0, 0))
        ...
        print("=== 엔딩 ===")
        print(message)
        # 실제 게임에서는 pygame 화면에 엔딩 이미지/텍스트 표시 코드로 대체

        # 엔딩 화면에서 더블클릭 시 페이드아웃 후 종료
        # 마지막 문구 출력 후, 마우스 클릭 1번 시 페이드아웃
        waiting = True
        while waiting:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
...
# --- 게임 루프 내 엔딩 체크 예시 ---
# 예시 game_data 생성 (실제 게임에서는 플레이어 상태에 맞게 값 할당)
# game_data = {
#     "money": player_money,
#     "play_time": play_time_seconds,  # 게임 시작 후 경과 시간(초)
#     "harvested": harvested_dict,      # 각 작물별 수확 여부
#     "tools_maxed": tools_maxed_bool,  # 모든 도구 최고 등급 여부
#     "days_survived": total_days_passed
# }
# 엔딩 체크 예시:
# if ending_manager.check_endings(game_data):
#     game_state = 'ENDING'  # 엔딩 상태로 전환
...
# 볼륨값을 settings.json에서 불러온 값으로 초기화
try:
    master_volume = settings.get("master_volume", 1)
    music_volume = settings.get("music_volume", 1)
    sfx_volume = settings.get("sfx_volume", 1)
except Exception:
    master_volume = 1
    music_volume = 1
    sfx_volume = 1
...
player_tools = {
    'watering_can': {
        ...
        'multiplier': 1,
        'owned_tool_ids': ['watering_can_rusted']
    },
    'scythe': {
        'id': 'scythe_basic',
        'multiplier': 1,
...
shop_items = [
    ...
    {"plant_type": "potato", "name": PLANT_INFO["potato"]["seed_name"], "price": 40000, "image": PLANT_INFO["potato"]["seed_image"], "description": "감자는 '만'ㅎ을수록 좋다."},
    {"plant_type": "pea", "name": PLANT_INFO["pea"]["seed_name"], "price": 400000, "image": PLANT_INFO["pea"]["seed_image"], "description": "신비한 힘이 깃든거 같다."},
...
tool_shop_items = {
    "watering_can": [
        ...
        {"id": "watering_can_better", "name": "좋아진 물뿌리개", "price": 750000, "multiplier": 8, "description": "성능이 향상된 물뿌리개입니다. 작물 성장을 촉진합니다.", "image": load_tool_image("watering_can_tier3.png")},
    ...
    "scythe": [
        ...
        {"id": "scythe_improved", "name": "좋아진 낫", "price": 5000000, "multiplier": 3, "description": "수확 효율이 눈에 띄게 좋아진 낫입니다. 농가의 희망이죠.", "image": load_tool_image("scythe_tier3.png")},
        ...
        {"id": "scythe_legendary", "name": "전설의 낫", "price": 300000000, "multiplier": 10, "description": "전설 속에 등장하는 낫입니다. 모든 작물을 풍요롭게 만듭니다.", "image": load_tool_image("scythe_tier6.png")},
    ]
}
...
def draw_shop_screen():
    ...
    for i, item in enumerate(shop_items):
        ...
        if item_y + SHOP_ITEM_HEIGHT > shop_item_list_rect.top and item_y < shop_item_list_rect.bottom:
            ...
            # 씨앗 이미지를 60x60으로 축소
            if item["image"]:
                scaled_img = pygame.transform.scale(item["image"], (60, 60))
                img_rect = scaled_img.get_rect(midleft=(item_rect_in_list.left + SHOP_ITEM_PADDING, item_rect_in_list.centery))
                screen.blit(scaled_img, img_rect)

            # 아이템 이름 (조금 오른쪽으로 조정)
            name_text = seed_name_font.render(item["name"], True, BLACK)
            name_text_rect = name_text.get_rect(midleft=(item_rect_in_list.left + SHOP_ITEM_PADDING + 90, item_rect_in_list.centery - 15))
...
def draw_tool_shop_screen(): # 기존 draw_crafting_screen에서 이름 변경
    ...
    for i, item in enumerate(current_tool_items):
        ...
        if item_y + SHOP_ITEM_HEIGHT > tool_shop_item_list_rect.top and item_y < tool_shop_item_list_rect.bottom:
            ...
            if item["image"]:
                scaled_img = pygame.transform.scale(item["image"], (80, 80))
                img_rect = scaled_img.get_rect(midleft=(item_rect_in_list.left + SHOP_ITEM_PADDING, item_rect_in_list.centery))
                screen.blit(scaled_img, img_rect)
            ...
            # 배율 텍스트는 기존 위치 유지
            multiplier_text = multiplier_text_font.render(f"배율: {item['multiplier']}{' 전체' if (item['id'] == 'watering_can_legendary' or item['id'] == 'scythe_legendary') else ''}", True, ORANGE)
    ...
    if selected_tool_shop_item_index != -1:
        ...
        if is_equipped:
            button_label = "장착중"
            button_color = TOOL_DISABLED_COLOR
        elif is_owned:
            button_label = "장착"
...
def draw_title_screen():
    ...
    # 설정 아이콘 (인게임과 동일한 위치: 오른쪽 아래)
    settings_icon_rect = pygame.Rect(WIDTH - SETTINGS_ICON_SIZE - 10, HEIGHT - SETTINGS_ICON_SIZE - 10, SETTINGS_ICON_SIZE, SETTINGS_ICON_SIZE)
    if settings_icon:
        screen.blit(settings_icon, settings_icon_rect)
    else:
        pygame.draw.rect(screen, GRAY, settings_icon_rect, border_radius=10)
    # 마우스 클릭 처리는 메인 루프에서 settings_icon_rect로 처리
...
def draw_game_screen():
    # 업적 UI 상태에서는 엔딩 이미지를 draw_game_screen에서 그리지 않음(겹침 방지)
    if game_state == ACHIEVEMENT_STATE:
        return
    # 튜토리얼 오버레이 통합
    if show_tutorial:
        overlay = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
        ...
        if tutorial_step == 7:
            ...
            try:
                tutorial_font = pygame.font.Font(font_path, 60)
                tutorial_sub_font = pygame.font.Font(font_path, 28)
            except:
                tutorial_font = pygame.font.SysFont('Galmuri14', 60)
                tutorial_sub_font = pygame.font.SysFont('Galmuri14', 28)
            tutorial_done_text = tutorial_font.render("튜토리얼 완료", True, YELLOW)
    ...
    # 업적 버튼
    global achievement_icon_rect
    if achievement_icon:
        achievement_icon_rect = achievement_icon.get_rect(bottomright=(WIDTH - 10 - SETTINGS_ICON_SIZE - icon_margin, HEIGHT - 10))
        screen.blit(achievement_icon, achievement_icon_rect)
    else:
        achievement_icon_rect = pygame.Rect(WIDTH - 10 - SETTINGS_ICON_SIZE - icon_margin, HEIGHT - 10 - ACHIEVEMENT_ICON_SIZE, ACHIEVEMENT_ICON_SIZE, ACHIEVEMENT_ICON_SIZE)
        pygame.draw.rect(screen, (200,200,0), achievement_icon_rect, border_radius=5)
    ...
    if game_state == ACHIEVEMENT_STATE:
        ...
        panel_height = 400
        panel_rect = pygame.Rect((WIDTH - panel_width)//2, (HEIGHT - panel_height)//2, panel_width, panel_height)
        pygame.draw.rect(screen, (255,255,255), panel_rect, border_radius=16)
        # 닫기 버튼 (오른쪽 상단)
        close_btn_size = 40
        close_btn_rect = pygame.Rect(panel_rect.right - close_btn_size - 16, panel_rect.top + 16, close_btn_size, close_btn_size)
        pygame.draw.rect(screen, (220,60,60), close_btn_rect, border_radius=8)
        x_font = pygame.font.SysFont('malgungothic', 32, bold=True)
        x_surf = x_font.render('X', True, (255,255,255))
        x_rect = x_surf.get_rect(center=close_btn_rect.center)
        screen.blit(x_surf, x_rect)
        # 업적(엔딩) 이미지들 가로 나열 (텍스트 없음)
        margin = 30
        total_width = ENDING_IMAGE_SIZE[0]*5 + margin*4
        start_x = panel_rect.left + (panel_width - total_width)//2
        y = panel_rect.top + 60
        for i, img in enumerate(ending_images_loaded):
            rect = img.get_rect()
            rect.topleft = (start_x + i*(ENDING_IMAGE_SIZE[0]+margin), y)
    ...
    if settings_icon:
        settings_icon_rect = settings_icon.get_rect(bottomright=(WIDTH - 10, HEIGHT - 10))
        screen.blit(settings_icon, settings_icon_rect)
    else:
        settings_icon_rect = pygame.Rect(WIDTH - 60, HEIGHT - 60, 50, 50)
        pygame.draw.rect(screen, BLUE, settings_icon_rect, border_radius=5)
    ...
    if show_tutorial:
        ...
        if tutorial_step == 1:
            ...
            # 하얀색 안내문과 동일한 줄 간격 및 위치로 맞춤
            font_height = button_font.get_height()
            line_gap = font_height + 8  # 하얀색 안내문과 동일하게(폰트 높이 + 8px 여유)
            base_y = watering_can_rect.top - (line_gap * 3) - 10  # 하얀색 안내문과의 간격도 동일하게
            # 자간(px)
            letter_spacing = 2  # 좌우 자간은 원래대로
            # 각 줄의 시작 y좌표
            line1_y = base_y
            line2_y = base_y + line_gap
            line3_y = base_y + line_gap * 2
            # 각 줄 텍스트
            line1 = "이게임에는 배율이란 시스템이 있습니다."
            line2 = "배율은 작물을 더 많이 자라게 하거나"
            line3 = "수확시 돈을 배율 만큼 더 많이줍니다."
            # 각 줄을 한 글자씩 자간을 띄워 그림
            draw_text_with_spacing(line1, button_font, YELLOW, (text_x, line1_y), letter_spacing)
            draw_text_with_spacing(line2, button_font, YELLOW, (text_x, line2_y), letter_spacing)
            draw_text_with_spacing(line3, button_font, YELLOW, (text_x, line3_y), letter_spacing)
...
def handle_game_screen_click(pos):
    ...
    global show_achievement_ui

    # 업적 UI 오버레이가 열려있으면, 오버레이 바깥 클릭 시 닫기
    if game_state == ACHIEVEMENT_STATE:
        # 닫기 버튼 클릭 시에만 닫히도록
        global achievement_close_btn_rect
        if 'achievement_close_btn_rect' in globals() and achievement_close_btn_rect.collidepoint(pos):
            show_achievement_ui = False
    ...
    # 업적 아이콘 클릭
    if achievement_icon_rect.collidepoint(pos):
        previous_game_state = game_state
        game_state = ACHIEVEMENT_STATE
        show_achievement_ui = True
        return
...
while running:
    ...
    for event in pygame.event.get():
                ...
                # R키: 씨앗목록(씨앗 선택 툴) 활성화 + 애니메이션
                    ...
                    if game_state == 'GAME':
                        ...
                        if show_seed_options:
                            ...
                            # 열리는 애니메이션
                            show_seed_options = True
                            seed_animation_target_offset = 1.0
                            seed_animation_current_offset = 0.0
                # 1~5키: 씨앗 선택
                elif event.key in [pygame.K_1, pygame.K_2, pygame.K_3, pygame.K_4, pygame.K_5]:
                    if game_state == 'GAME' and active_tool_on_click == 'seed_select' and show_seed_options:
        ...
        if event.type == pygame.QUIT:
            ...
            if event.button == 1: # 좌클릭
                if game_state == 'TITLE':
                    ...
                    if settings_icon_rect.collidepoint(event.pos):
                        game_state = 'SETTINGS'
                        previous_game_state = 'TITLE'
                    elif start_button_rect.collidepoint(event.pos):
                        import os
                        save_path = os.path.join(BASE_DIR, "data", "save_data.json")
                        if os.path.exists(save_path):
                            # 경고 팝업 띄우기
                            confirm_font = pygame.font.SysFont('malgungothic', 36, bold=True)
                            confirm_text = "정말 처음부터 시작하시겠습니까?\n(기존 저장 데이터가 삭제됩니다)"
                            confirm_surf = pygame.Surface((600, 220), pygame.SRCALPHA)
                            confirm_surf.fill((255,255,255,240))
                            border_rect = confirm_surf.get_rect(center=(WIDTH//2, HEIGHT//2))
                            pygame.draw.rect(confirm_surf, (180,60,60), confirm_surf.get_rect(), 4, border_radius=0)
                            lines = confirm_text.split('\n')
                            for i, line in enumerate(lines):
                                txt = confirm_font.render(line, True, (60,60,60))
                                txt_rect = txt.get_rect(center=(300, 60 + i*60))
                                confirm_surf.blit(txt, txt_rect)
                            # 버튼
                            import os
                            galmuri_font_path = os.path.join(BASE_DIR, "assets", "fonts", "Galmuri14.ttf")
                            confirm_font = pygame.font.Font(galmuri_font_path, 24)
                            confirm_text = "정말 처음부터 시작하시겠습니까? (기존 저장 데이터가 삭제됩니다)"
                            import textwrap
                            lines = textwrap.wrap(confirm_text, width=18)
                            confirm_surf = pygame.Surface((600, 220))
                            confirm_surf.fill((255,255,255))
                            border_rect = confirm_surf.get_rect(center=(WIDTH//2, HEIGHT//2))
                            pygame.draw.rect(confirm_surf, (180,60,60), confirm_surf.get_rect(), 4, border_radius=0)
                            for i, line in enumerate(lines):
                                txt = confirm_font.render(line, True, (60,60,60))
                                txt_rect = txt.get_rect(center=(300, 60 + i*40))
                                confirm_surf.blit(txt, txt_rect)
                            # 버튼
                            btn_font = pygame.font.Font(galmuri_font_path, 22)
                            yes_rect = pygame.Rect(120, 150, 120, 50)
                            no_rect = pygame.Rect(360, 150, 120, 50)
                            pygame.draw.rect(confirm_surf, (60,180,80), yes_rect, border_radius=8)
                            pygame.draw.rect(confirm_surf, (180,60,60), no_rect, border_radius=8)
                            yes_txt = btn_font.render("예", True, (255,255,255))
                            no_txt = btn_font.render("아니오", True, (255,255,255))
                            confirm_surf.blit(yes_txt, yes_txt.get_rect(center=yes_rect.center))
                            confirm_surf.blit(no_txt, no_txt.get_rect(center=no_rect.center))
                            confirming = True
                            while confirming:
                                draw_title_screen()
                                screen.blit(confirm_surf, border_rect)
                                pygame.display.update()
                                for event2 in pygame.event.get():
                                    ...
                                    if event2.type == pygame.MOUSEBUTTONDOWN:
                                        mx, my = event2.pos
                                        rel_x, rel_y = mx - border_rect.left, my - border_rect.top
                ...
                elif game_state == 'TOOL_SHOP':
                    handle_tool_shop_screen_click(event.pos)
                    if scrollbar_handle_rect and scrollbar_handle_rect.collidepoint(event.pos):
                        is_dragging_scrollbar = True
                        scrollbar_drag_offset_y = event.pos[1] - scrollbar_handle_rect.y
                elif game_state == ACHIEVEMENT_STATE:
                    # 닫기 버튼
                    if 'achievement_close_btn_rect' in globals() and achievement_close_btn_rect.collidepoint(event.pos):
                        show_achievement_ui = False
                        game_state = previous_game_state
                        achievement_selected_idx = None
                    # 초기화 버튼 클릭 시
                    elif 'achievement_reset_btn_rect' in globals() and achievement_reset_btn_rect.collidepoint(event.pos):
                        # 경고 팝업 띄우기
                        import os
                        galmuri_font_path = os.path.join(BASE_DIR, "assets", "fonts", "Galmuri14.ttf")
                        confirm_font = pygame.font.Font(galmuri_font_path, 24)
                        confirm_text = "정말 엔딩 업적을 초기화하시겠습니까? (되돌릴 수 없습니다)"
                        # 텍스트 줄바꿈 처리 (최대 18자 기준)
                        import textwrap
                        lines = textwrap.wrap(confirm_text, width=18)
                        # Surface를 완전 투명하게 만들고, 흰색 배경은 테두리보다 4px 작게 그림
                        confirm_surf = pygame.Surface((520, 180), pygame.SRCALPHA)
                        confirm_surf.fill((0,0,0,0))
                        border_rect = confirm_surf.get_rect(center=(WIDTH//2, HEIGHT//2))
                        # 빨간 테두리(가장자리, 각진 사각형)
                        pygame.draw.rect(confirm_surf, (220,60,60), confirm_surf.get_rect(), 4, border_radius=0)
                        # 흰색 배경(테두리 안쪽, 각진 사각형)
                        inner_rect = confirm_surf.get_rect().inflate(-8, -8)
                        pygame.draw.rect(confirm_surf, (255,255,255), inner_rect, border_radius=0)
                        for i, line in enumerate(lines):
                            txt = confirm_font.render(line, True, (60,60,60))
                            txt_rect = txt.get_rect(center=(260, 48 + i*32))
                            confirm_surf.blit(txt, txt_rect)
                        # 버튼
                        btn_font = pygame.font.Font(galmuri_font_path, 22)
                        yes_rect = pygame.Rect(60, 120, 120, 44)
                        no_rect = pygame.Rect(340, 120, 120, 44)
                        pygame.draw.rect(confirm_surf, (60,180,80), yes_rect, border_radius=8)
                        pygame.draw.rect(confirm_surf, (180,60,60), no_rect, border_radius=8)
                        yes_txt = btn_font.render("예", True, (255,255,255))
                        no_txt = btn_font.render("아니오", True, (255,255,255))
                            ...
                            for event2 in pygame.event.get():
                                ...
                                if event2.type == pygame.MOUSEBUTTONDOWN:
                                    ...
                                    if yes_rect.collidepoint(rel_x, rel_y):
                                        ...
                                        confirming = False
                            pygame.time.delay(16)
                    # 엔딩 이미지 클릭 시 조건 문구 표시
                    elif 'achievement_img_rects' in globals():
                        for idx, rect in enumerate(achievement_img_rects):
                            if rect.collidepoint(event.pos):
                                achievement_selected_idx = idx
                                break
                        else:
                            achievement_selected_idx = None
            ...
            if event.key == pygame.K_F2:
                fast_day_mode = not fast_day_mode
                if fast_day_mode:
                    DAY_CHANGE_INTERVAL = 0.05  # 0.05초마다 날짜 변경 (더 빠르게)
                else:
                    DAY_CHANGE_INTERVAL = original_day_change_interval
                print(f"빠른 날짜 모드: {fast_day_mode}")
    ...
    if game_state == 'GAME' and not show_tutorial:
        update_date()
        # 엔딩 조건 체크 및 바로 엔딩 화면 전환
        game_data = {
            "money": player_money,
            "play_time": int(time.time() - game_start_time),  # 게임 시작 후 경과 시간(초)
            "days": total_days_passed,
        }
        if ending_manager.check_endings(game_data):
            game_state = 'ENDING'
    elif show_tutorial:
